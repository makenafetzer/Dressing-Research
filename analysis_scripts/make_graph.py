...
def make_logg_CM(campaign, proposal, folder):
    ...
    '''
    df_all = pd.read_csv('../csv_files/%s/master_search.csv' %campaign)

    df_big_proposal = pd.read_csv('../csv_files/%s/big_targets/prop_%d.csv' %(campaign, proposal))
#Would want the above line to cycle through big_targets
    ...
    plt.plot(j_min_h_dd, J_dd, '.', c='blue', alpha=0.3)
    plt.plot(j_min_h_gg, J_gg, '.', c='green', alpha=0.3)
